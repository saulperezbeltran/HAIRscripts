...
#SBATCH --job-name=SiO2_1_3      #Set the job name to "JobExample2"
#SBATCH --time=30:00:00               #Set the wall clock limit to 6hr and 30min
...
#Cool down
rm -fr $cwd/02_Cooldown
echo "ReaxFF Step"
mkdir -p $cwd/02_Cooldown/00_reaxFF
cd $cwd/01_Annealing
last=$(ls -dv  */*01_*  | tail -1 | cut -d"-" -f1 | cut -d"/" -f3)
echo "Last Annealing Step Identified: $last"

cd $cwd/02_Cooldown/00_reaxFF
cp $cwd/01_Annealing/$last-cycle/01_AIMD/CONTCAR POSCAR.vasp
VaspTwoLammps POSCAR.vasp
cp $DE1500K/{cooldown.lmp,2014-Aluminosilicates.reaxff} .
LoadLammps
GetLastFrameL2V
echo "Vasp Step"
mkdir -p $cwd/02_Cooldown/01_AIMD
cp CONTCAR.vasp $cwd/02_Cooldown/01_AIMD/POSCAR
cd $cwd/02_Cooldown/01_AIMD
