...
#SBATCH --job-name=SiO2_3_4      #Set the job name to "JobExample2"
#SBATCH --time=24:00:00               #Set the wall clock limit to 6hr and 30min
...
##Equilibration with absorbed water
echo "Vasp Step"
mkdir -p $cwd/03_AbsorbedWater/00_AIMD
cd $cwd/03_AbsorbedWater
cp POSCAR.vasp $cwd/03_AbsorbedWater/00_AIMD/POSCAR
cd $cwd/03_AbsorbedWater/00_AIMD/
cp $DE1500K/INCAR_roomT INCAR
cp $DE1500K/{KPOINTS,POTCAR} .
LoadVasp

echo "ReaxFF Step"
mkdir -p $cwd/03_AbsorbedWater/00_reaxFF
cp CONTCAR $cwd/03_AbsorbedWater/00_reaxFF/POSCAR.vasp
cd $cwd/03_AbsorbedWater/00_reaxFF
VaspTwoLammps POSCAR.vasp
cp $DE1500K/{AbsorbedWater.lmp,2014-Aluminosilicates.reaxff} .
LoadLammps
GetLastFrameL2V

echo "Vasp Step"
mkdir -p $cwd/03_AbsorbedWater/01_AIMD
cp CONTCAR.vasp $cwd/03_AbsorbedWater/01_AIMD/POSCAR
cd $cwd/03_AbsorbedWater/01_AIMD
cp $DE1500K/INCAR_roomT INCAR
cp $DE1500K/{KPOINTS,POTCAR} .
LoadVasp
